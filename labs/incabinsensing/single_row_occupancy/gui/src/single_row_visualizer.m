...
if(SETUP_VIA_GUI)
    ...
    ENABLE_RECORD = 0;
    datFile.path = '';
    datFile.name = 'incabin_demo_uart_stream.txt';
    cfgFile.path = '';
    cfgFile.name = 'chirp.cfg';
    logFile.path = '';
    logFile.name = ['incabin_demo_log_' datestr(now,'mmddyy_HHMM') '.dat'];
    offset.height = 2;
    offset.az = 0;
    offset.el = -20;
    comPort.cfg = 51;
...
NUM_ZONES = 3;
for idx = 1:NUM_ZONES
    stateMachine(idx).state = 0;
    stateMachine(idx).freeze = 0;
    stateMachine(idx).detect2freeCount = 0;
end
ZoneCountOccupied = 0;
...
hFig = figure('Name', 'Rear Seat Occupancy Visualizer V1.0.0','Color','white','CloseRequestFcn',@plotfig_closereq);

% init plot axes
maxRange = max([calc_P.rangeMax_m]);
[hFig, hAx3D] = init3DPlot(hFig,[-maxRange maxRange],[0 maxRange], [0 offset.height], zone_P);
...
% init handle for occupancy textbox
styleOccupancy = {'LineStyle','none', 'FontUnits','normalized','FontSize',0.025,'FitBoxToText','off'};
occupancyString = {'PlaceHolder'};
hOccupancy = annotation(hFig,'textbox',[0.6 0.8 0.3 0.2], 'String', occupancyString, styleOccupancy{:},'Color','k'); 

% init uicontrols
selectView = uicontrol(hFig,'Style','popupmenu',...
    'Position',[10 10 100 25],...
    'String',{'3D View','X-Y View','Y-Z View','X-Z View'},...
    'Callback',@(selectView,event) selection(selectView,hAx3D));
selection(selectView, hAx3D);
...
if(REAL_TIME_MODE)
    ...
    READ_MODE = 'ALL';
    [allFrames, bytesBuffer, bytesBufferLen, numFramesAvailable,validFrame] = parseBytes(bytesBuffer, bytesBufferLen, READ_MODE);
...
while (RUN_VIZ)    
    ...
    if(REAL_TIME_MODE)
        ...
        [newframe, bytesBuffer, bytesBufferLen, numFramesAvailable,validFrame] = parseBytes(bytesBuffer, bytesBufferLen, READ_MODE);
        frameIndex = 1;
    else
    ...
    if(validFrame(frameIndex))
        ...
        zoneStatus = {'Unoccupied','Unoccupied','Unoccupied'};
        if(1)
            ...
            if(HAVE_VALID_PT_CLOUD)
                numPoints = newframe.detObj.numDetectedObj;
            else
                numPoints = 0;
            end
           ...
           BORESIGHT_MIN = 0.005;
            if(HAVE_VALID_PT_CLOUD)
                ...
                % Boresight filter for points with coordinates (0,y,0)
                for pt =1:numPoints
                    if((abs(newframe.detObj.x(pt)) < BORESIGHT_MIN) && (abs(newframe.detObj.z(pt)) < BORESIGHT_MIN))
                        newframe.detObj.x(pt) = [];
                        newframe.detObj.y(pt) = [];
                        newframe.detObj.z(pt) = [];
                        newframe.detObj.doppler(pt) = [];
                        newframe.sideInfo.snr(pt) = [];
                        numPoints = numPoints - 1;
                        newframe.detObj.numDetectedObj = numPoints;
                        if(pt <= numPoints)
                            break
                        end
                    end
                end
                % Pt cloud hasn't been transformed based on offset
                ...
                cartPoints = [hPtCloud.XData; hPtCloud.YData; hPtCloud.ZData; newframe.sideInfo.snr];
            else
                hPtCloud.XData = [];
                hPtCloud.YData = [];
                hPtCloud.ZData = [];
            ...
            if(OCCUPANCY_DETECTION == 1)
                zoneMap = zoneAssign(numPoints, cartPoints, NUM_ZONES, zone_P);
                [ZoneCountOccupied, stateMachine] = occupancyDetection(NUM_ZONES, stateMachine, cartPoints, zoneMap, P.occStateMach);
            end
            ...
            OccupancyString = strings(3);
            for zoneIdx =1:NUM_ZONES
                if(stateMachine(zoneIdx).state == 1)
                    OccupancyString(zoneIdx) = sprintf('Seat %d Occupied', zoneIdx);
                elseif(stateMachine(zoneIdx).state == 0)
                    OccupancyString(zoneIdx) = sprintf('Seat %d Unoccupied', zoneIdx);
                else
                    OccupancyString(zoneIdx) = sprintf('Seat %d Unknown', zoneIdx);
                end
            end
            hOccupancy.String = OccupancyString;
