...
void ODDemo_Heatmap_steeringVecGen(ODDemo_DataPathObj *obj)
{
    ...
    for (i = 0; i < obj->steeringVecSize; i++)
    {
        ...
        for (j = 2; j < obj->nRxAnt; j++)
        {
            ...
            obj->steeringVec[(obj->nRxAnt - 1) * i + j - 1].real = (float)frealJ;
            obj->steeringVec[(obj->nRxAnt - 1) * i + j - 1].imag = (float)fimagJ;
        }
    }
}


void ODDemo_Heatmap_aoaEstCaponBF(ODDemo_DataPathObj *obj)
{
...
void ODDemo_Heatmap_aoaEstCaponBF_covInv(
         uint8_t   invFlag,
         ...
         cplxf_t  *invRnMatrices)
{
    ...
    if (clutterRmFlag)
    {
        ...
        for (antIdx = 0; antIdx < nRxAnt; antIdx++)
        {
            ...
            mmwavelib_vecsum((int16_t *) input,
                             (int32_t *) sumVal,
                             (int32_t) nChirps);
            ...
            mmwavelib_vecsubc((int16_t *) input,
                              (int16_t *) input,
                              (uint32_t) meanVal,
                              (int32_t) nChirps);
        }
 ...
 *
 */
void ODDemo_Heatmap_aoaEstCaponBF_heatmap(uint8_t  bfFlag,
                                          int32_t  nRxAnt,
...
//This function compares the current heatmap to the previous frame's heatmap,
//cell by cell against a threshold, smoothing the power from frame to frame.
//The heatmap max is then recalculated.
void ODDemo_Heatmap_smoothing(float *current_heatmap, float *previous_heatmap)
{
  uint32_t  idx;
  float     thresh, newmax;
  float    *curr_cell = current_heatmap;
  float    *prev_cell = previous_heatmap;

  newmax = 0;

  for (idx = 0; idx < ODDEMO_ANGLEHEATMAP_BUF_SIZE; idx++)
  {
    thresh = *prev_cell++ * oddemo_parms.smoothing;
...
void ODDemo_Heatmap_scale_heatmap(float *heatin, uint16_t *heatout)
{
  uint16_t  idx;
  float     scale;
  ...
  if (oddemo_heatmap_max <= (float)ODDEMO_HEATMAP_MAX) //no scaling needed
  {
    for (idx = 0; idx < ODDEMO_ANGLEHEATMAP_BUF_SIZE; idx++)
      *out++ = (uint16_t) *ptr++;
  }
  ...
  {
    scale = (float)ODDEMO_HEATMAP_MAX / oddemo_heatmap_max;
